        ...
        for i, val in enumerate(df[col_name]):
            try:
                ...
                if isinstance(triple, list) and len(triple) >= 2:
                    extracted.extend([triple[1]])  # Get first and second entries
                else:
...
def run_training():
    ...
    for i, energy in enumerate(data['energy_output']):
        data['energy_output'][i] = data['energy_output'][i]
    ...
    for i, graph in enumerate(data['node_features']):
        ...
        # Keep only the first 3 features per node
        graph_array = graph_array[:, :7]
    ...
    batch_size = num_train  # Adjust as needed
    ...
    if save_option == 'y':
        ...
        model_dir = "../Energy Compound GNN Models"  # Save the trained model in a folder along with its hyperparameters.
